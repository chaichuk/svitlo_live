...
`**Подальші ітерації були вимушеним кроком через ненадійні дані сайту svitlo.live та прохання користувачів. Тому на даний час інтеграція використовує власне міні АРІ для регіонів ДТЕК (Київська, Дніпропетровська, Одеська області), Ясно (м. Київ, м.Дніпро) та Львівської областей.**`
...
2. **`SvitloCoordinator` (coordinator.py)**  
   ...
   - Розумний вибір проксі: Автоматично перемикається між стандартним API (для більшості областей) та спеціалізованим проксі-сервером (Київська, Дніпропетровська, Одеська, Львівська області, м. Київ, м.Дніпро) для забезпечення максимальної достовірності графіків.
...
### Для регіонів ДТЕК (Київ, Одеса, Дніпро) + Ясно (м. Київ, м.Дніпро) та Львівської області:
...
Важливо для Дніпра: При виборі регіону "м. Дніпро", вам буде запропоновано обрати оператора системи розподілу (ОСР):

 - Дніпро (ДнЕМ) - Дніпровські електромережі

 - Дніпро (ЦЕК) - Центральна енергетична компанія
...
## Разом з інтеграцією автоматично встановлюється декілька blueprint для сповіщень про графік електропостачання та зміни протягом дня.
### Налаштування:
